...
import { ArrowLeft, MapPin, Grid3X3, List, Zap, Building2, Home, Layers } from 'lucide-react'
...
const TYPE_CONFIG: Record<string, { 
  ...
  residential: {
    ...
    description: 'Find your dream home from our collection of apartments, villas, and houses',
    icon: Home,
    queryField: 'property_type',
    queryValues: ['apartment', 'villa', 'house', 'flat', 'penthouse', 'duplex', 'studio', 'independent', 'row house', 'bungalow', 'farmhouse']
  },
  commercial: {
    name: 'Commercial Properties',
    description: 'Premium commercial spaces for offices, shops, and businesses',
    icon: Building2,
    queryField: 'property_type',
    queryValues: ['office', 'shop', 'commercial', 'showroom', 'warehouse', 'retail', 'sco', 'scf', 'multiplex']
  },
  plots: {
    name: 'Plots & Land',
    description: 'Prime plots and land parcels for your dream project or investment',
    icon: Layers,
    queryField: 'property_type',
    queryValues: ['plot', 'land', 'agricultural', 'industrial land']
  }
}
...
export default function PropertyTypePage() {
  ...
  const config = TYPE_CONFIG[typeSlug] || TYPE_CONFIG.residential
  const Icon = config.icon

  // Build query string
  const queryString = config.queryValues.map(v => `property_type=${v}`).join('&')
  
  const { data, isLoading } = useSWR(
    `/api/properties?${queryString}&sort=${sortBy}&limit=30`,
    fetcher,
        ...
        <div className="luxury-section bg-white border-b border-[var(--luxury-border)]">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-2 mb-6">
              <Link href="/" className="text-sm text-gray-600 hover:text-[var(--luxury-gold)] transition-colors">
                Home
              </Link>
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[var(--luxury-gold)] to-[var(--luxury-navy)] flex items-center justify-center">
                  <Icon className="h-5 w-5 text-white" />
                </div>
                <div>
                  <span className="text-[var(--luxury-gold)] text-xs font-semibold uppercase tracking-wider">Premium Selection</span>
                  <h1 className="text-2xl md:text-3xl font-bold text-[var(--luxury-navy)]">{config.name}</h1>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    router.push(`/properties/${typeSlug}?${query.toString()}`)
                  }}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 mb-6">
                We don&apos;t have any {config.name.toLowerCase()} available right now.
              </p>
              ...
              {properties.map((property) => (
                <div key={property._id} className="luxury-card p-0 flex overflow-hidden h-48">
                  ...
                  <div className="flex-1 p-6 flex flex-col justify-between">
                    <div>
                      <Link href={`/properties/${typeSlug}/${property.slug || property._id}`}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link href={`/properties/${typeSlug}/${property.slug || property._id}`} className="luxury-button">
                        View Details
                      </Link>
                    </div>
                  </div>
